...
void ECSManager::connectParameters(entt::entity source, const std::string& sourceParam,
    ...
    if (m_registry.all_of<blot::components::Parameter>(source) && m_registry.all_of<blot::components::Parameter>(target)) {
        auto& sourceParamComp = m_registry.get<blot::components::Parameter>(source);
        auto& targetParamComp = m_registry.get<blot::components::Parameter>(target);
...
void ECSManager::createNode(const std::string& nodeType, float x, float y) {
    ...
    node.type = blot::components::NodeType::Custom; // or map nodeType string to enum if needed
    node.name = nodeType;
...
void ECSManager::connectNodes(entt::entity sourceNode, const std::string& output,
    ...
    if (m_registry.all_of<blot::components::NodeComponent>(sourceNode) && m_registry.all_of<blot::components::NodeComponent>(targetNode)) {
        auto& sourceNodeComp = m_registry.get<blot::components::NodeComponent>(sourceNode);
        auto& targetNodeComp = m_registry.get<blot::components::NodeComponent>(targetNode);
        
        // Add output to source node (using pins)
        auto outputs = sourceNodeComp.getOutputs();
        if (std::find_if(outputs.begin(), outputs.end(), [&](const blot::components::NodePin& pin){ return pin.name == output; }) == outputs.end()) {
            // Add a new output pin if not found
            sourceNodeComp.pins.push_back({output, "float", false, true, 0.0f, "Output"});
        }
        // Add input to target node (using pins)
        auto inputs = targetNodeComp.getInputs();
        if (std::find_if(inputs.begin(), inputs.end(), [&](const blot::components::NodePin& pin){ return pin.name == input; }) == inputs.end()) {
            // Add a new input pin if not found
            targetNodeComp.pins.push_back({input, "float", true, false, 0.0f, "Input"});
        }
    }
}
...
void ECSManager::updateAnimationSystem(float deltaTime) {
    auto view = m_registry.view<blot::components::Animation, blot::components::Transform>();
...
void ECSManager::renderShapeSystem() {
    // Rendering now handled by runShapeRenderingSystem using an external IRenderer.
}
...
void ECSManager::updateParameterSystem() {
    ...
    for (auto entity : view) {
        ...
        if (parameter.isConnected && m_registry.valid(parameter.connectedTo)) {
            if (m_registry.all_of<blot::components::Parameter>(parameter.connectedTo)) {
                auto& connectedParam = m_registry.get<blot::components::Parameter>(parameter.connectedTo);
...
blot::json ECSManager::getSettings() const {
    ...
    for (auto entity : m_entities) {
        ...
        // TODO: Serialize Transform component once json conversion helper exists
        // if (m_registry.all_of<blot::components::Transform>(entity)) { ... }
        // Repeat for other component types as needed...
        // Optionally, add entity name if available
...
void ECSManager::setSettings(const blot::json& settings) {
    clear();
    if (!settings.contains("entities")) return;
