...
void ShapeRenderingSystem(ECSManager &ecs,
	...
	auto view =
		ecs.view<components::Transform, components::Shape, components::Style>();
...
void renderRectangle(const components::Transform &transform,
					 const components::Shape &shape,
					 const components::Style &style,
					 std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
		return;
...
void renderEllipse(const components::Transform &transform,
				   const components::Shape &shape,
				   const components::Style &style,
				   std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
...
void renderLine(const components::Transform &transform,
				const components::Shape &shape, const components::Style &style,
				std::shared_ptr<IRenderer> renderer) {
...
void renderPolygon(const components::Transform &transform,
				   const components::Shape &shape,
				   const components::Style &style,
				   std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
...
void renderStar(const components::Transform &transform,
				const components::Shape &shape, const components::Style &style,
				std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
		return;
...
void renderSelectionOverlay(ECSManager &ecs, const ImVec2 &canvasPos,
							const ImVec2 &canvasSize,
							std::shared_ptr<IRenderer> renderer) {
	auto view = ecs.view<components::Transform, components::Shape,
...
void renderDrawingPreview(ECSManager &ecs, const ImVec2 &canvasPos,
						  const ImVec2 &canvasSize,
						  std::shared_ptr<IRenderer> renderer) {
	auto view =
		ecs.view<components::Transform, components::Shape, components::Style>();
...
void setFillStyle(const components::Style &style,
				  std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
		return;
...
void setStrokeStyle(const components::Style &style,
					std::shared_ptr<IRenderer> renderer) {
	if (!renderer)
		return;
