...
void Canvas::renderECSShapes() {
    ...
    auto view = m_ecs->view<blot::components::Transform, blot::components::Shape, blot::components::Style>();
    ...
    for (auto entity : allEntities) {
        bool hasTransform = m_ecs->hasComponent<blot::components::Transform>(entity);
...
void Canvas::switchRenderer(RendererType type) {
    ...
    // Remove or replace the line: auto newRenderer = addonManager->createRenderer(type);
    // The original code had this line, but the user's edit hint implies its removal.
    // Assuming the user's intent is to remove this line.
    // If the user intended to replace it with a new renderer, they would need to provide the new renderer object.
    // For now, we'll just log an error if the renderer type is not found.
    spdlog::error("switchRenderer: Renderer type {} not found or implemented.", static_cast<int>(type));
}
...
void Canvas::setSettings(const json& settings) {
    if (settings.contains("width")) m_width = settings["width"].get<int>();
    if (settings.contains("height")) m_height = settings["height"].get<int>();
    if (settings.contains("name")) m_name = settings["name"].get<std::string>();
    ...
    if (settings.contains("samples")) m_settings.samples = settings["samples"].get<int>();
