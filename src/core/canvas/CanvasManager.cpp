...
std::shared_ptr<Canvas> CanvasManager::createCanvas(const CanvasSettings& settings, const std::string& name) {
...
void CanvasManager::setSettings(const blot::json& settings) {
    clear();
    if (settings.contains("activeCanvasIndex")) m_activeCanvasIndex = settings["activeCanvasIndex"];
    if (settings.contains("canvases")) {
        for (const auto& cj : settings["canvases"]) {
            CanvasSettings cs;
            if (cj.contains("width")) cs.width = cj["width"];
            if (cj.contains("height")) cs.height = cj["height"];
            if (cj.contains("r")) cs.r = cj["r"];
            if (cj.contains("g")) cs.g = cj["g"];
            if (cj.contains("b")) cs.b = cj["b"];
            if (cj.contains("a")) cs.a = cj["a"];
            if (cj.contains("samples")) cs.samples = cj["samples"];
