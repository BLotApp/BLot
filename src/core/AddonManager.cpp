...
blot::AddonManager::AddonManager()
...
std::shared_ptr<blot::AddonBase> blot::AddonManager::getAddon(const std::string& name) const {
    auto it = m_addons.find(name);
    if (it != m_addons.end()) {
        return it->second;
    }
...
void blot::AddonManager::scanAddonDirectory(const std::string& directory) {
        ...
        if (entry.is_directory()) {
            ...
            if (std::filesystem::exists(configFile)) {
                ...
                if (!meta.name.empty()) {
                    ...
                    spdlog::warn("Addon at '{}' has no name in addon.json", addonPath);
                }
            } else {
                spdlog::warn("No addon.json found for addon '{}', skipping.", addonName);
            }
        }
    }
    ...
    std::function<void(const std::string&)> visit = [&](const std::string& name) {
        ...
        if (visiting.count(name)) {
            spdlog::error("Circular dependency detected involving addon: {}", name);
            hasError = true;
            return;
        }
        ...
        if (it != allMeta.end()) {
            for (const auto& dep : it->second.dependencies) {
                if (allMeta.find(dep) == allMeta.end()) {
                    spdlog::error("Missing dependency: '{}' required by addon '{}'", dep, name);
                    hasError = true;
                } else {
                    visit(dep);
                }
    ...
    if (hasError) {
        spdlog::error("Dependency resolution failed due to missing or circular dependencies. Addon loading order may be incomplete.");
    }
    m_addonOrder = order;
}
...
std::vector<std::shared_ptr<blot::AddonBase>> blot::AddonManager::getEnabledAddons() const {
...
std::vector<std::shared_ptr<blot::AddonBase>> blot::AddonManager::getAllAddons() const {
...
std::vector<std::string> blot::AddonManager::getAddonDependencies(const std::string& name) const {
    auto addon = getAddon(name);
    if (addon) {
...
void blot::AddonManager::addGlobalEventListener(const std::string& event, std::function<void()> callback) {
...
void blot::AddonManager::setSettings(const blot::json& settings) {
    if (settings.contains("addonDirectory")) m_addonDirectory = settings["addonDirectory"];
    if (settings.contains("addonConfig")) m_addonConfig = settings["addonConfig"].get<std::unordered_map<std::string, bool>>();
    if (settings.contains("addonOrder")) m_addonOrder = settings["addonOrder"].get<std::vector<std::string>>();
