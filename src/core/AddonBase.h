...
public:
    ...
    void setGraphics(std::shared_ptr<Graphics> graphics) { m_graphics = graphics; }
    void setScriptEngine(std::shared_ptr<bxScriptEngine> scriptEngine) { m_scriptEngine = scriptEngine; }
    
    // Utility functions for addons
    template<typename T>
    T* getAddon(const std::string& name);
...
template<typename T>
T* AddonBase::getAddon(const std::string& name) {
