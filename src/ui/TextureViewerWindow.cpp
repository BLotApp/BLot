...
void TextureViewerWindow::setTexture(unsigned int textureId, int width, int height) {
...
void TextureViewerWindow::handleMouseInput() {
    ...
    m_textureEnd = ImVec2(m_texturePos.x + m_textureSize.x, m_texturePos.y + m_textureSize.y);
    ImVec2 mousePos = ImGui::GetMousePos();
    m_mouseInsideTexture = (mousePos.x >= m_texturePos.x && mousePos.x <= m_textureEnd.x &&
...
ImVec2 TextureViewerWindow::getTextureSize() const {
    return m_textureSize;
}

std::string TextureViewerWindow::getName() const {
    return m_title;
}

std::string TextureViewerWindow::getTitle() const {
    return m_title;
}
...
void TextureViewerWindow::drawTexture() {
    if (m_hasTexture && m_textureId != 0) {
        ...
        ImGui::Image(reinterpret_cast<void*>(static_cast<uintptr_t>(m_textureId)), displaySize, ImVec2(0, 0), ImVec2(1, 1));
        if (ImGui::IsWindowHovered()) {
            ImGui::SetTooltip("Texture: %dx%d", m_textureWidth, m_textureHeight);
        }
    } else {
        ImVec2 availableSize = ImGui::GetContentRegionAvail();
...
ImVec2 TextureViewerWindow::convertToTextureCoordinates(const ImVec2& screenPos) const {
    if (!m_hasTexture || m_textureWidth == 0 || m_textureHeight == 0) {
        return ImVec2(0, 0);
    }
    float x = (screenPos.x - m_texturePos.x) / m_textureSize.x * m_textureWidth;
    float y = (screenPos.y - m_texturePos.y) / m_textureSize.y * m_textureHeight;
    return ImVec2(x, y);
}
