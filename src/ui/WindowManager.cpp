...
WindowManager::WindowManager() : m_focusedWindowEntity(entt::null) {
	...
	spdlog::debug("[DEBUG] WindowManager constructed, workspaceDir={}",
				  m_workspaceDir);
	loadExistingWorkspaces();
}

WindowManager::~WindowManager() { closeAllWindows(); }

entt::entity WindowManager::createWindow(const std::string &name,
...
std::shared_ptr<Window> WindowManager::getFocusedWindow() {
	if (m_focusedWindowEntity != entt::null &&
		...
		auto &windowComp =
			m_registry.get<WindowComponent>(m_focusedWindowEntity);
		return windowComp.window;
	}
	return nullptr;
}
...
WindowManager::getAllWindowsWithDisplayNames() const {
...
void WindowManager::renderAllWindows() {
	...
	auto view = m_registry.view<WindowComponent, WindowTransformComponent,
								WindowStyleComponent>();
	for (auto entity : view) {
		auto &windowComp = view.get<WindowComponent>(entity);
...
void WindowManager::updateFocus() {
	...
	if (newFocusedEntity != m_focusedWindowEntity) {
		...
		if (newFocusedEntity != entt::null) {
			auto &windowComp =
				m_registry.get<WindowComponent>(newFocusedEntity);
			windowComp.isFocused = true;
		}
	}
}
...
// Remove all usages of WindowSettingsComponent, setWindowSettings, and
// getWindowSettings
...
WindowManager::getWindowsByCategory(const std::string &category) {
	std::vector<std::string> categoryWindows;
	auto view = m_registry.view<WindowComponent>();
	for (auto entity : view) {
		auto &windowComp = view.get<WindowComponent>(entity);
		// The category is no longer stored in WindowComponent, so this will
		// always return empty This function needs to be updated to reflect the
		// new WindowComponent structure For now, returning an empty vector as a
		// placeholder
...
WorkspaceConfig
WindowManager::captureCurrentUIState(const std::string &workspaceName) {
...
WorkspaceConfig WindowManager::createWorkspaceFromCurrentState(
	const std::string &workspaceName) {
	return captureCurrentUIState(workspaceName);
}

bool WindowManager::createWorkspace(const std::string &workspaceName,
...
std::vector<std::pair<std::string, std::string>>
WindowManager::getAvailableWorkspacesWithNames() const {
...
WorkspaceConfig
WindowManager::getWorkspaceConfig(const std::string &workspaceName) const {
...
void WindowManager::setWindowVisibility(const std::string &windowName,
										bool visible) {
...
std::string
WindowManager::getWorkspaceConfigPath(const std::string &workspaceName) const {
