...
void UIManager::setupWindows(BlotEngine *app) {
	...
	auto addonManagerWindow = std::make_shared<AddonManagerWindow>(
		"Addon Manager###AddonManager", Window::Flags::None);
...
void UIManager::setWindowVisibility(const std::string &windowName,
									bool visible) {
	if (m_windowManager) {
...
bool UIManager::loadWorkspace(const std::string &workspaceName) {
	std::cout << "[UIManager] loadWorkspace called with: " << workspaceName
			  << std::endl;
	if (m_windowManager) {
...
UIManager::getAvailableWorkspaces() const {
	if (m_windowManager) {
...
void UIManager::showModal(const std::string &title, const std::string &message,
