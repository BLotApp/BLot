...
void StrokeWindow::renderBasicSettings() {
    ...
    // Stroke Cap
    const char* capItems[] = {
        "Butt", "Square", "Round", "Round Reversed", 
        "Triangle", "Triangle Reversed"
    };
    int currentCap = static_cast<int>(m_strokeCap);
    if (ImGui::Combo("Cap Style", &currentCap, capItems, IM_ARRAYSIZE(capItems))) {
        setStrokeCap(static_cast<BLStrokeCap>(currentCap));
    }
    
    // Stroke Join
    const char* joinItems[] = {
        "Miter Clip", "Miter Bevel", "Miter Round", 
        "Bevel", "Round"
    };
    int currentJoin = static_cast<int>(m_strokeJoin);
    if (ImGui::Combo("Join Style", &currentJoin, joinItems, IM_ARRAYSIZE(joinItems))) {
    ...
    if (m_strokeJoin <= BL_STROKE_JOIN_MITER_ROUND) {
        ...
        if (ImGui::SliderFloat("Miter Limit", &miterLimit, 0.1f, 20.0f, "%.1f")) {
...
void StrokeWindow::renderAdvancedSettings() {
    ...
    // Transform Order
    const char* transformItems[] = {
        "Transform After Stroke", "Transform Before Stroke"
    };
    int currentTransform = static_cast<int>(m_transformOrder);
    if (ImGui::Combo("Transform Order", &currentTransform, transformItems, IM_ARRAYSIZE(transformItems))) {
        setTransformOrder(static_cast<BLStrokeTransformOrder>(currentTransform));
    }
...
void StrokeWindow::renderDashArrayEditor() {
    ...
    if (!m_dashArrayInputValid) {
        ImGui::PopStyleColor();
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Invalid dash pattern");
    }
...
void StrokeWindow::renderDashPresets() {
    ...
    const char* presets[] = {
        "Solid", "Dashed", "Dotted", "Dash-Dot", "Long Dash", "Double Dash"
    };
...
std::string StrokeWindow::strokeCapToString(BLStrokeCap cap) const {
    switch (cap) {
        case BL_STROKE_CAP_BUTT: return "Butt";
        case BL_STROKE_CAP_SQUARE: return "Square";
        case BL_STROKE_CAP_ROUND: return "Round";
        case BL_STROKE_CAP_ROUND_REV: return "Round Reversed";
        case BL_STROKE_CAP_TRIANGLE: return "Triangle";
        case BL_STROKE_CAP_TRIANGLE_REV: return "Triangle Reversed";
...
std::string StrokeWindow::strokeJoinToString(BLStrokeJoin join) const {
    switch (join) {
        case BL_STROKE_JOIN_MITER_CLIP: return "Miter Clip";
        case BL_STROKE_JOIN_MITER_BEVEL: return "Miter Bevel";
        case BL_STROKE_JOIN_MITER_ROUND: return "Miter Round";
        case BL_STROKE_JOIN_BEVEL: return "Bevel";
        case BL_STROKE_JOIN_ROUND: return "Round";
        default: return "Unknown";
    }
}

std::string StrokeWindow::transformOrderToString(BLStrokeTransformOrder order) const {
    switch (order) {
        case BL_STROKE_TRANSFORM_ORDER_AFTER: return "Transform After Stroke";
        case BL_STROKE_TRANSFORM_ORDER_BEFORE: return "Transform Before Stroke";
        default: return "Unknown";
    }
} 