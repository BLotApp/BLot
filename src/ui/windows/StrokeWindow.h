...
class StrokeWindow : public blot::Window {
  public:
	...
	using StrokeDashArrayCallback =
		std::function<void(const std::vector<double> &)>;
	using StrokeDashOffsetCallback = std::function<void(double)>;
	using StrokeTransformOrderCallback =
		std::function<void(BLStrokeTransformOrder)>;

	StrokeWindow(const std::string &title = "Stroke Window",
				 blot::Window::Flags flags = blot::Window::Flags::None);
	...
	BLStrokeTransformOrder getTransformOrder() const {
		return m_transformOrder;
	}
	...
	// Callback setters
	void setStrokeWidthCallback(StrokeWidthCallback callback) {
		m_strokeWidthCallback = callback;
	}
	void setStrokeCapCallback(StrokeCapCallback callback) {
		m_strokeCapCallback = callback;
	}
	void setStrokeJoinCallback(StrokeJoinCallback callback) {
		m_strokeJoinCallback = callback;
	}
	void setMiterLimitCallback(StrokeMiterLimitCallback callback) {
		m_miterLimitCallback = callback;
	}
	void setDashArrayCallback(StrokeDashArrayCallback callback) {
		m_dashArrayCallback = callback;
	}
	void setDashOffsetCallback(StrokeDashOffsetCallback callback) {
		m_dashOffsetCallback = callback;
	}
	void setTransformOrderCallback(StrokeTransformOrderCallback callback) {
		m_transformOrderCallback = callback;
	}
