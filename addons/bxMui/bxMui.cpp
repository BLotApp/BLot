...
bxMui::bxMui() : AddonBase("Mui", "0.1.0") {}

bool bxMui::init() {
	auto engine = m_blotEngine; // inherited protected member
	if (!engine) {
		spdlog::error(
			"[bxMui] BlotEngine pointer not set – cannot initialise UI");
		return false;
	}
	if (engine->getUIManager()) {
		spdlog::info("[bxMui] UI manager already present – skipping creation");
