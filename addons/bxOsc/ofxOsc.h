...
class bxOsc : public AddonBase {
  public:
	...
	// Event handling
	void onMessageReceived(const std::string &address,
  ...
  private:
	...
	// Event callbacks
	std::unordered_map<std::string, std::function<void(const OscMessage &)>>
		m_addressCallbacks;
	std::vector<std::function<void(const OscMessage &)>> m_generalCallbacks;

	// Network simulation (in a real implementation, this would use actual OSC
	// libraries)
